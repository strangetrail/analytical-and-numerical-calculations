...
#define BASEPORT0 0x3f8 /* ttyS0 */
#define BASEPORT1 0x2f8 /* ttyS0 */
#define BASEPORT2 0x3e8 /* ttyS0 */
#define BASEPORT3 0x2e8 /* ttyS0 */
#define BASEPORT4 0xeff0 /* ttyS0 */
#define BASEPORT BASEPORT0 /* ttyS0 */
#define PARAMSCOUNT 8
...
int main(int Parm_Count, char *Parms[])
{
   ...
   if (Parm_Count==PARAMSCOUNT)  /*if there are the right number of parameters on the command line*/
   {
   ...
   if ((Parm_Count==PARAMSCOUNT) && (error==0))  /*if the command line entrys were correct*/
   {                                    /*run the program*/
      ...
      while (count < 1) /*TEMP*/
      {
        ...
        byte = 0xFF; /*TEMP*/
        sscanf( Param_strings[6], "%i", &iBasePort ); /*TEMP*/
        switch ( iBasePort )
        {
         case 0:
           iBasePort = BASEPORT0;
           ...
           if (status==1)  /*if a key was hit*/
           {
              ...
              { /* branch to appropiate key handler */
                 case 0x1b: /* Esc */
                    ...
                    break;
                 default:
                    /*fputc((int) Key,output);*/ /*TEMP*/
  /*                  sprintf(message,"%x ",Key); */  /*debug*/
  /*                  fputs(message,output); */
                    /* Get access to the ports */
                    if (ioperm(iBasePort, 3, 1)) {perror("ioperm"); exit(1);}
                    
                    while ( count < 10000000 )
                    {
                      write(fd,&Keys,128);          /*write 1 byte to the port*/
                      count = count + 1;
                      /*if ( count % 50 == 0 )
                      {
                        Key = ~Key;
                    ...
                    if (ioperm(iBasePort, 3, 0)) {perror("ioperm"); exit(1);}
  ...
  /*            if (res==1) STOP=TRUE; */ /* stop loop if only a CR was input */
        ...
        /*idx = 0;*/ /*TEMP*/
        /*for (idx = 0; idx < 5; idx = idx + 1)*/ /*TEMP*/
        /*{*/
          /*usleep(100000);*/ /*TEMP*/
        /*}*/
        /*printf( "\n\tResetting port descriptor...  \n\n" );*/ /*TEMP*/
